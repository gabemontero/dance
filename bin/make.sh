...
release() {
  ...
  $(git -C "$HELM_CHART" log --format="  - %s" "$previous_version^..$version" -- Chart.yaml values.yaml templates)
  "
  echo git push
  git tag --force "$version"
  echo git push --tags
  cd -
  echo rm -rf "$HELM_REPOSITORY"
}
...
_get_versions() {
  ...
  for previous_version in $(
    git log --simplify-by-decoration --format=%D 0.2.1^ |
      sed -e 's|, tag: |\n|g' -e 's|^tag: ||' |
